...
from services.user_service import create_user, create_user_agent, delete_user, get_all_users, get_user_by_id, update_user
...
def createUser(user: User):
    ...
    user.password = Hasher.get_password_hash(user.password) # type: ignore
    return create_user(
        user=user
)

@router.post("/create-agent", response_model=BaseAPIResponse)
def createUserAgent(user: User):
    return create_user_agent(user)
